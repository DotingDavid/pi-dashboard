...
class DashboardApp:
    ...
    def _select_autocomplete_command(self):
        """Select from cascading menu or filtered list"""
        categories = [
            ('Dashboard', ['/home', '/tasks', '/kanban', '/sync', '/weather', '/screenshot', '/restart', '/quit']),
            ('Local', ['/clear', '/sessions', '/new', '/status', '/size', '/help']),
            ('OpenClaw', ['/compact', '/think', '/model', '/reasoning']),
        ]
        
        if self.chat_input == '/':
            # Cascading menu mode
            cat_idx = getattr(self, 'cmd_cat_idx', 0)
            cmd_idx = getattr(self, 'cmd_autocomplete_idx', 0)
            in_submenu = getattr(self, 'cmd_in_submenu', False)
            
            if in_submenu:
                # Select command from submenu
                _, cmds = categories[cat_idx]
                if cmd_idx < len(cmds):
                    self.chat_input = cmds[cmd_idx]
                    self.chat_cursor = len(self.chat_input)
            else:
                # Enter submenu on Tab
            ...
            matches = [c for c in all_cmds if c.startswith(self.chat_input.lower())]
    ...
    def _draw_command_autocomplete(self, input_y):
        """Draw cascading command menu - categories on left, commands on right"""
        categories = [
            ('Dashboard', (100, 200, 150), [
                ('/home', 'Home screen'),
                ('/tasks', 'Tasks panel'),
                ('/kanban', 'Kanban board'),
                ('/sync', 'Refresh all'),
        ...
        # Only show when just "/" typed
        if self.chat_input != '/':
            # Filter mode - show matching commands flat
            all_cmds = []
            for cat_name, cat_color, cmds in categories:
                for cmd, desc in cmds:
                    if cmd.startswith(self.chat_input.lower()):
                        all_cmds.append((cmd, desc, cat_color))
            ...
            popup_w = 240
            popup_h = min(len(all_cmds), 8) * 24 + 8
            popup_x = 15
            ...
            idx = getattr(self, 'cmd_autocomplete_idx', 0)
            idx = min(idx, len(all_cmds) - 1)
            self.cmd_autocomplete_idx = idx
            
            y = popup_y + 4
            for i, (cmd, desc, color) in enumerate(all_cmds[:8]):
                is_sel = (i == idx)
                if is_sel:
                    pygame.draw.rect(self.screen, (50, 70, 110), (popup_x + 4, y, popup_w - 8, 22), border_radius=4)
                cmd_surf = self.fonts['status'].render(cmd, True, (200, 220, 255) if is_sel else (140, 160, 200))
                self.screen.blit(cmd_surf, (popup_x + 10, y + 4))
                desc_surf = self.fonts['status'].render(desc, True, color if is_sel else (80, 95, 120))
                self.screen.blit(desc_surf, (popup_x + 100, y + 4))
                y += 24
            return
        
        # Initialize state
        if not hasattr(self, 'cmd_cat_idx'):
            self.cmd_cat_idx = 0
        if not hasattr(self, 'cmd_in_submenu'):
            self.cmd_in_submenu = False
        
        cat_idx = self.cmd_cat_idx
        in_submenu = self.cmd_in_submenu
        cmd_idx = getattr(self, 'cmd_autocomplete_idx', 0)
        
        # Left popup - categories
        left_w = 100
        left_h = len(categories) * 28 + 8
        left_x = 15
        ...
        y = left_y + 4
        for i, (cat_name, cat_color, _) in enumerate(categories):
            is_sel = (i == cat_idx)
            if is_sel and not in_submenu:
                pygame.draw.rect(self.screen, (50, 70, 110), (left_x + 4, y, left_w - 8, 26), border_radius=4)
            elif is_sel:
                pygame.draw.rect(self.screen, (40, 55, 85), (left_x + 4, y, left_w - 8, 26), border_radius=4)
            
            cat_surf = self.fonts['status'].render(cat_name, True, cat_color if is_sel else (100, 115, 140))
            self.screen.blit(cat_surf, (left_x + 10, y + 6))
            
            # Arrow indicator
            if is_sel:
                arrow_surf = self.fonts['status'].render(">", True, cat_color)
                self.screen.blit(arrow_surf, (left_x + left_w - 18, y + 6))
            
            y += 28
        
        # Right popup - commands for selected category
        _, cat_color, cmds = categories[cat_idx]
        
        right_w = 180
        right_h = len(cmds) * 24 + 8
        ...
        cmd_idx = min(cmd_idx, len(cmds) - 1)
        self.cmd_autocomplete_idx = cmd_idx
        
        y = right_y + 4
        for i, (cmd, desc) in enumerate(cmds):
            is_sel = in_submenu and (i == cmd_idx)
            if is_sel:
                pygame.draw.rect(self.screen, (50, 70, 110), (right_x + 4, y, right_w - 8, 22), border_radius=4)
            
            cmd_surf = self.fonts['status'].render(cmd, True, (200, 220, 255) if is_sel else (120, 140, 180))
            self.screen.blit(cmd_surf, (right_x + 10, y + 4))
            
            desc_surf = self.fonts['status'].render(desc, True, (140, 155, 180) if is_sel else (70, 85, 110))
            self.screen.blit(desc_surf, (right_x + 75, y + 4))
        ...
        if ctrl and event.key == pygame.K_a:
            ...
            if self.chat_input.startswith('/'):
                if self.chat_input == '/' and getattr(self, 'cmd_in_submenu', False):
                    # Navigate commands in submenu
                    self.cmd_autocomplete_idx = max(0, getattr(self, 'cmd_autocomplete_idx', 0) - 1)
                elif self.chat_input == '/':
                    # Navigate categories
                    self.cmd_cat_idx = max(0, getattr(self, 'cmd_cat_idx', 0) - 1)
                    self.cmd_autocomplete_idx = 0
                else:
                    # Filtered list
                    self.cmd_autocomplete_idx = max(0, getattr(self, 'cmd_autocomplete_idx', 0) - 1)
            ...
            if self.chat_input.startswith('/'):
                if self.chat_input == '/' and getattr(self, 'cmd_in_submenu', False):
                    ...
                    # Navigate categories
                    self.cmd_cat_idx = min(2, getattr(self, 'cmd_cat_idx', 0) + 1)
                    self.cmd_autocomplete_idx = 0
                else:
                    # Filtered list
                    self.cmd_autocomplete_idx = min(7, getattr(self, 'cmd_autocomplete_idx', 0) + 1)
