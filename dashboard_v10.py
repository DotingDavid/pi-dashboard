#!/usr/bin/env python3
"""
Pi 400 Dashboard v10 - Multi-Panel Edition
For 3.5" Waveshare TFT (480x320)

Panels:
1. Dashboard - Clock, weather, status
2. Tasks - Todoist integration
3. Commands - Quick action buttons
4. Chat - Flat list chat interface
...
class DashboardApp:
    def __init__(self):
        ...
        # Auto-start terminal if launching in terminal mode
        if self.mode == MODE_COMMANDS:
            self.terminal.start()
    ...
    def _friendly_session_name(self, key, session_data):
        ...
        # agent:main:discord:channel:123 -> "Discord #bot-chat"
        # agent:main:pi-display -> "Pi Display"
        # agent:main:openai:xxx -> "API Session"
        ...
        if ':main' in key and key.endswith(':main'):
            ...
            # Extract channel name from displayName if available
            if '#' in display:
    ...
    def draw_dashboard(self):
        ...
        # Hint
        hint = "F2: Tasks  F3: Terminal  F4: Chat"
    ...
    def draw_tasks(self):
        ...
        if self.task_editing:
            ...
            hint = "N: New | Enter: Edit | Space: Done | P: Priority | Del: Remove | Shift+↑↓: Move"
        hint_surf = self.fonts['status'].render(hint, True, C['text_muted'])
    ...
    def draw_commands(self):
        ...
        commands = [
            ('Restart Gateway', 'gateway restart', C['warning']),
            ('Update Gateway', 'gateway update', C['accent']),
            ('Reboot Pi', 'sudo reboot', C['error']),
            ('Shutdown Pi', 'sudo shutdown -h now', C['error']),
            ('Gateway Status', 'openclaw status', C['success']),
            ('Node Status', 'openclaw nodes status', C['success']),
        ]
        
        # Calculate button layout
        button_height = 42
        button_margin = 8
        button_width = SCREEN_WIDTH - button_margin * 2
        
        y = y_start + 12
        
        for idx, (label, cmd, color) in enumerate(commands):
            # Button background
            button_rect = (button_margin, y, button_width, button_height)
            
            # Highlight if selected (for future keyboard nav)
            bg_color = C['bg_item_hover'] if idx == getattr(self, 'selected_cmd', -1) else C['bg_item']
            pygame.draw.rect(self.screen, bg_color, button_rect, border_radius=8)
            pygame.draw.rect(self.screen, color, button_rect, width=2, border_radius=8)
            
            # Label
            label_surf = self.fonts['msg'].render(label, True, C['text_bright'])
            label_x = button_margin + 12
            label_y = y + (button_height - label_surf.get_height()) // 2
            self.screen.blit(label_surf, (label_x, label_y))
            
            y += button_height + button_margin
        
        # Instructions
        help_text = "Press number (1-6) to run command"
        help_surf = self.fonts['status'].render(help_text, True, C['text_dim'])
        help_x = (SCREEN_WIDTH - help_surf.get_width()) // 2
        self.screen.blit(help_surf, (help_x, SCREEN_HEIGHT - 24))
    ...
    def draw_chat(self):
        ...
        input_box_width = SCREEN_WIDTH - 16 - 32  # Box width minus padding
        ...
        if self.chat_input:
            ...
            if full_width > input_box_width:
                ...
                # If cursor is past visible area, shift start position
                if cursor_x > input_box_width - 20:
                    # Find how many chars we need to drop from start
                    offset = 0
    ...
    def _draw_bubble(self, msg, y, min_y):
        ...
        for i, line in enumerate(lines):
            # First line uses role color for prefix, rest use normal text color
            if i == 0:
                # Render prefix in color, rest in white
                prefix_surf = self.fonts['msg'].render(prefix, True, text_color)
                self.screen.blit(prefix_surf, (margin, ty))
                
                # Render text after prefix
    ...
    def _draw_chat_menu(self):
        ...
        for i, item in enumerate(items):
            ...
            # Add checkmark for current session
    ...
    def _handle_chat_key(self, event):
        if self.chat_menu_open:
            if event.key == pygame.K_ESCAPE:
                ...
                if hasattr(self, '_sessions_loading') and self._sessions_loading:
                    return
                # Don't allow selecting "loading" placeholder
        ...
        if event.key == pygame.K_RETURN:
            ...
            self.chat_scroll = min(self.chat_scroll + 2, max(0, len(self.messages) - 2))
        elif event.key == pygame.K_DOWN:
    ...
    def _handle_commands_key(self, event):
        ...
        # Command list (matching draw_commands)
        commands = [
            ('Restart Gateway', 'systemctl --user restart openclaw-gateway'),
            ('Update Gateway', 'cd ~/.openclaw && git pull && npm install'),
        ...
        # Number key handling (1-6)
        if event.unicode and event.unicode in '123456':
            idx = int(event.unicode) - 1
            if idx < len(commands):
                label, cmd = commands[idx]
