...
class Settings:
    def __init__(self):
        ...
        self.task_ui_style = 1       # 1=Linear, 2=Superhuman, 3=Things
        self.load()
        ...
        try:
            if SETTINGS_FILE.exists():
                with open(SETTINGS_FILE) as f:
                    ...
                    self.task_ui_style = data.get('task_ui_style', 1)
        except:
            pass
        ...
        try:
            ...
            with open(SETTINGS_FILE, 'w') as f:
                json.dump({
                    ...
                    'task_ui_style': self.task_ui_style
                }, f)
        except:
            pass
...
class DashboardApp:
    ...
    def draw_tabs(self):
        ...
        # Version indicator (and style when in Tasks mode)
        if self.mode == MODE_TASKS:
            style_names = {1: 'Lin', 2: 'Spd', 3: 'Thg'}
            style_name = style_names.get(self.settings.task_ui_style, 'Lin')
            ver_text = f"v14 [{style_name}]"
        else:
            ver_text = "v14"
        ver_surf = self.fonts['status'].render(ver_text, True, C['text_muted'])
    ...
    def draw_tasks(self):
        """Dispatch to selected UI style"""
        style = self.settings.task_ui_style
        if style == 1:
            self.draw_tasks_linear()
        elif style == 2:
            self.draw_tasks_superhuman()
        elif style == 3:
            self.draw_tasks_things()
        else:
            self.draw_tasks_linear()
    
    def draw_tasks_linear(self):
        """Style 1: Linear Focus - Professional, keyboard hints, info-dense"""
        import math
        ...
        urgent_count = len([t for t in all_tasks if t.get('priority', 1) == 4])
        
        # ═══════════════════════════════════════════════════════════════
        # HEADER SECTION - Premium Stats Bar
        # ═══════════════════════════════════════════════════════════════
        header_y = 46
        
        # Stats bar background
        stats_h = 52
        pygame.draw.rect(self.screen, (32, 34, 45), (10, header_y, SCREEN_WIDTH - 20, stats_h), border_radius=12)
        
        # Individual stat items
        stats = [
            {'label': 'ALL', 'value': total, 'color': (100, 140, 220)},
            {'label': 'TODAY', 'value': today_count, 'color': (220, 180, 60)},
            {'label': 'LATE', 'value': overdue_count, 'color': (220, 90, 90)},
            {'label': 'URGENT', 'value': urgent_count, 'color': (220, 100, 160)},
        ]
        
        stat_w = (SCREEN_WIDTH - 40) // 4
        stat_x = 20
        
        for i, stat in enumerate(stats):
            # Divider line (except first)
            if i > 0:
                pygame.draw.line(self.screen, (50, 52, 65), 
                               (stat_x - 5, header_y + 12), (stat_x - 5, header_y + stats_h - 12), 1)
            
            # Value - large and prominent
            val_text = str(stat['value'])
            val_surf = self.fonts['menu_title'].render(val_text, True, stat['color'])
            val_x = stat_x + stat_w // 2 - val_surf.get_width() // 2
            self.screen.blit(val_surf, (val_x, header_y + 8))
            
            # Label - smaller below
            label_surf = self.fonts['status'].render(stat['label'], True, C['text_muted'])
            label_x = stat_x + stat_w // 2 - label_surf.get_width() // 2
            self.screen.blit(label_surf, (label_x, header_y + 32))
            
            stat_x += stat_w
        
        # ═══════════════════════════════════════════════════════════════
        # FILTER TABS - Pill style with counts
        # ═══════════════════════════════════════════════════════════════
        tab_y = header_y + stats_h + 10
        tabs = [
            {'id': 'all', 'label': 'All', 'count': total, 'color': (100, 140, 220)},
            {'id': 'today', 'label': 'Today', 'count': today_count, 'color': (220, 180, 60)},
            {'id': 'overdue', 'label': 'Overdue', 'count': overdue_count, 'color': (220, 90, 90)},
        ]
        
        tab_x = 12
        for tab in tabs:
            is_active = self.task_filter == tab['id']
            
            # Calculate width based on content
            label_surf = self.fonts['msg'].render(tab['label'], True, C['text_bright'])
            count_surf = self.fonts['status'].render(str(tab['count']), True, C['text_bright'])
            tab_w = label_surf.get_width() + count_surf.get_width() + 32
            tab_h = 30
            
            if is_active:
                # Active tab - filled with color
                pygame.draw.rect(self.screen, tab['color'], (tab_x, tab_y, tab_w, tab_h), border_radius=15)
                text_color = (20, 20, 25)
                count_bg = (255, 255, 255, 80)
            else:
                # Inactive - outline only
                pygame.draw.rect(self.screen, (55, 57, 70), (tab_x, tab_y, tab_w, tab_h), border_radius=15)
                pygame.draw.rect(self.screen, (70, 72, 85), (tab_x, tab_y, tab_w, tab_h), width=1, border_radius=15)
                text_color = C['text_dim']
            
            # Label
            label_surf = self.fonts['msg'].render(tab['label'], True, text_color)
            self.screen.blit(label_surf, (tab_x + 12, tab_y + 7))
            
            # Count badge
            count_x = tab_x + tab_w - count_surf.get_width() - 12
            if is_active:
                pygame.draw.rect(self.screen, (255, 255, 255), (count_x - 4, tab_y + 6, count_surf.get_width() + 8, 18), border_radius=9)
                count_surf = self.fonts['status'].render(str(tab['count']), True, tab['color'])
            else:
                count_surf = self.fonts['status'].render(str(tab['count']), True, C['text_muted'])
            self.screen.blit(count_surf, (count_x, tab_y + 9))
            
            tab_x += tab_w + 10
        
        # Sync indicator - modern style
        sync_status = getattr(self, 'todoist_sync_status', 'live')
        sync_x = SCREEN_WIDTH - 50
        sync_y = tab_y + 8
        
        if sync_status == 'live':
            # Green ring with checkmark
            pygame.draw.circle(self.screen, C['success'], (sync_x, sync_y + 7), 10, width=2)
            # Mini checkmark
            pygame.draw.line(self.screen, C['success'], (sync_x - 4, sync_y + 7), (sync_x - 1, sync_y + 10), 2)
            pygame.draw.line(self.screen, C['success'], (sync_x - 1, sync_y + 10), (sync_x + 5, sync_y + 3), 2)
        elif sync_status == 'syncing':
            # Animated spinner
            angle = time.time() * 6
            for i in range(8):
                a = angle + i * math.pi / 4
                intensity = int(100 + 155 * ((i / 8)))
                color = (intensity, intensity, 50)
                x = sync_x + int(7 * math.cos(a))
                y = sync_y + 7 + int(7 * math.sin(a))
                pygame.draw.circle(self.screen, color, (x, y), 2)
        else:
            # Red X
            pygame.draw.circle(self.screen, C['error'], (sync_x, sync_y + 7), 10, width=2)
            pygame.draw.line(self.screen, C['error'], (sync_x - 4, sync_y + 3), (sync_x + 4, sync_y + 11), 2)
            pygame.draw.line(self.screen, C['error'], (sync_x + 4, sync_y + 3), (sync_x - 4, sync_y + 11), 2)
        
        # ═══════════════════════════════════════════════════════════════
        # TASK LIST - Build display list with expanded subtasks
        # ═══════════════════════════════════════════════════════════════
        list_y = tab_y + 42
        card_h = 50
        card_gap = 4
        ...
        # Build display list (tasks + expanded subtasks)
        if not hasattr(self, 'task_expanded'):
            self.task_expanded = set()
        ...
        if not display_list:
            # Empty state - clean and modern
            empty_y = list_y + 50
            
            # Card
            pygame.draw.rect(self.screen, (35, 37, 48), (120, empty_y, SCREEN_WIDTH - 240, 110), border_radius=20)
            
            # Animated checkmark circle
            check_x = SCREEN_WIDTH // 2
            check_y = empty_y + 40
            pulse = 0.9 + 0.1 * math.sin(time.time() * 2)
            radius = int(22 * pulse)
            pygame.draw.circle(self.screen, (60, 180, 100), (check_x, check_y), radius, width=3)
            pygame.draw.line(self.screen, (60, 180, 100), (check_x - 8, check_y + 2), (check_x - 2, check_y + 8), 3)
            pygame.draw.line(self.screen, (60, 180, 100), (check_x - 2, check_y + 8), (check_x + 10, check_y - 6), 3)
            
            if self.task_filter == 'all':
                msg = "No tasks yet"
                hint = "Press N to add your first task"
            else:
                msg = f"No {self.task_filter} tasks"
                hint = "You're all caught up!"
            
            msg_surf = self.fonts['title'].render(msg, True, C['text_bright'])
            hint_surf = self.fonts['status'].render(hint, True, C['text_dim'])
            self.screen.blit(msg_surf, ((SCREEN_WIDTH - msg_surf.get_width())//2, empty_y + 68))
            self.screen.blit(hint_surf, ((SCREEN_WIDTH - hint_surf.get_width())//2, empty_y + 90))
        else:
            # Scroll handling
            ...
            if self.task_scroll > 0:
                for i in range(3):
                    y_off = i * 3
                    pygame.draw.line(self.screen, (100, 140, 220), 
                                   (SCREEN_WIDTH//2 - 20 + i*5, list_y - 8 + y_off),
                                   (SCREEN_WIDTH//2, list_y - 14 + y_off), 2)
                    pygame.draw.line(self.screen, (100, 140, 220),
                                   (SCREEN_WIDTH//2, list_y - 14 + y_off),
                                   (SCREEN_WIDTH//2 + 20 - i*5, list_y - 8 + y_off), 2)
            ...
            for i, item in enumerate(visible):
                ...
                subtasks = task.get('subtasks', [])
                is_expanded = task['id'] in self.task_expanded
                
                # Priority colors - vibrant
                p_colors = {
                    4: (235, 85, 85),   # Red - urgent
                    3: (235, 175, 55),  # Orange - high
                    2: (85, 145, 235),  # Blue - medium  
                    1: (95, 100, 120)   # Gray - normal
                }
                ...
                indent_px = indent * 30
                card_x = 10 + indent_px
                card_w = SCREEN_WIDTH - 20 - indent_px
                
                # Subtask connector line
                if indent > 0:
                    pygame.draw.line(self.screen, (55, 58, 72), 
                                   (card_x - 15, card_y - card_gap), 
                                   (card_x - 15, card_y + card_h // 2), 2)
                    pygame.draw.line(self.screen, (55, 58, 72),
                                   (card_x - 15, card_y + card_h // 2),
                                   (card_x, card_y + card_h // 2), 2)
                
                # Selection glow effect
                if is_selected:
                    for g in range(3, 0, -1):
                        glow_color = (p_color[0], p_color[1], p_color[2])
                        pygame.draw.rect(self.screen, glow_color, 
                                       (card_x - g*2, card_y - g*2, card_w + g*4, card_h + g*4), 
                                       border_radius=14 + g, width=2)
                
                # Card background
                bg_base = (48, 50, 62) if is_selected else (35, 37, 48)
                if indent > 0:
                    bg_base = (42, 44, 55) if is_selected else (32, 34, 44)
                pygame.draw.rect(self.screen, bg_base, (card_x, card_y, card_w, card_h), border_radius=12)
                
                # Top highlight line for depth
                pygame.draw.line(self.screen, (60, 62, 75), 
                               (card_x + 12, card_y + 1), (card_x + card_w - 12, card_y + 1), 1)
                
                # Priority indicator - stylish left accent
                pygame.draw.rect(self.screen, p_color, (card_x, card_y, 5, card_h), 
                ...
                if subtasks and indent == 0:
                    exp_x = card_x + 18
                    exp_y = card_y + card_h // 2
                    if is_expanded:
                        # Down chevron (expanded)
                        pygame.draw.line(self.screen, (130, 135, 155), (exp_x - 5, exp_y - 3), (exp_x, exp_y + 3), 2)
                        pygame.draw.line(self.screen, (130, 135, 155), (exp_x, exp_y + 3), (exp_x + 5, exp_y - 3), 2)
                    else:
                        # Right chevron (collapsed)
                        pygame.draw.line(self.screen, (130, 135, 155), (exp_x - 3, exp_y - 5), (exp_x + 3, exp_y), 2)
                        pygame.draw.line(self.screen, (130, 135, 155), (exp_x + 3, exp_y), (exp_x - 3, exp_y + 5), 2)
                    cb_offset = 20
                else:
                    cb_offset = 0
                
                # Checkbox - double ring style
                cb_x = card_x + 35 + cb_offset
                cb_y = card_y + card_h // 2
                
                # Smaller checkbox for subtasks
                cb_radius = 12 if indent == 0 else 10
                pygame.draw.circle(self.screen, (55, 58, 72), (cb_x, cb_y), cb_radius)
                pygame.draw.circle(self.screen, p_color, (cb_x, cb_y), cb_radius, width=2)
                
                # Inner indicator based on priority
                if priority == 4:
                    pygame.draw.circle(self.screen, p_color, (cb_x, cb_y), cb_radius - 4)
                    excl = self.fonts['status'].render("!", True, (255, 255, 255))
                    self.screen.blit(excl, (cb_x - excl.get_width()//2, cb_y - excl.get_height()//2))
                elif priority == 3:
                    pygame.draw.circle(self.screen, p_color, (cb_x, cb_y), cb_radius - 5, width=2)
                elif priority == 2:
                    pygame.draw.circle(self.screen, p_color, (cb_x, cb_y), 3)
                
                # Content
                text_x = cb_x + cb_radius + 12
                
                if is_selected and self.task_editing:
                    edit_text = self.task_edit_text[:40]
                    text_surf = self.fonts['msg'].render(edit_text, True, C['text_bright'])
                    self.screen.blit(text_surf, (text_x, card_y + 15))
                    
                    cursor_x = text_x + self.fonts['msg'].size(self.task_edit_text[:self.task_edit_cursor])[0]
                    if int(time.time() * 2) % 2:
                        pygame.draw.rect(self.screen, C['accent'], (cursor_x, card_y + 13, 2, 22), border_radius=1)
                else:
                    # Task text - show subtask count
                    max_len = 28 if due else 38
                    if indent > 0:
                        max_len -= 4
                    display_text = content[:max_len] + ('…' if len(content) > max_len else '')
                    text_surf = self.fonts['msg'].render(display_text, True, C['text_bright'])
                    self.screen.blit(text_surf, (text_x, card_y + 15))
                    
                    # Subtask count badge
                    if subtasks and indent == 0:
                        sub_count = len(subtasks)
                        sub_text = f"{sub_count} sub"
                        sub_surf = self.fonts['status'].render(sub_text, True, (120, 125, 145))
                        sub_x = text_x + text_surf.get_width() + 10
                        pygame.draw.rect(self.screen, (50, 52, 62), (sub_x, card_y + 15, sub_surf.get_width() + 10, 18), border_radius=9)
                        self.screen.blit(sub_surf, (sub_x + 5, card_y + 17))
                    
                    # Due date pill
                    if due:
                        due_lower = due.lower()
                        if 'overdue' in due_lower or 'yesterday' in due_lower:
                            due_bg, due_fg, due_icon = (200, 65, 65), (255, 255, 255), "!"
                        elif 'today' in due_lower:
                            due_bg, due_fg, due_icon = (220, 170, 50), (30, 30, 35), "●"
                        elif 'tomorrow' in due_lower:
                            due_bg, due_fg, due_icon = (70, 130, 200), (255, 255, 255), "→"
                        else:
                            due_bg, due_fg, due_icon = (55, 58, 72), (160, 165, 180), ""
                        
                        due_text = due[:8]
                        if due_icon:
                            due_text = due_icon + " " + due_text
                        due_surf = self.fonts['status'].render(due_text, True, due_fg)
                        due_w = due_surf.get_width() + 16
                        due_x = card_x + card_w - due_w - 10
                        due_y = card_y + (card_h - 22) // 2
                        pygame.draw.rect(self.screen, due_bg, (due_x, due_y, due_w, 22), border_radius=11)
                        self.screen.blit(due_surf, (due_x + 8, due_y + 5))
            ...
            if remaining > 0:
                ...
                for i in range(3):
                    y_off = i * 3
                    pygame.draw.line(self.screen, (100, 140, 220),
                                   (SCREEN_WIDTH//2 - 20 + i*5, arrow_y + y_off),
                                   (SCREEN_WIDTH//2, arrow_y + 6 + y_off), 2)
                    pygame.draw.line(self.screen, (100, 140, 220),
                                   (SCREEN_WIDTH//2, arrow_y + 6 + y_off),
                                   (SCREEN_WIDTH//2 + 20 - i*5, arrow_y + y_off), 2)
                remain_surf = self.fonts['status'].render(f"+{remaining} more", True, (100, 140, 220))
                self.screen.blit(remain_surf, (SCREEN_WIDTH//2 - remain_surf.get_width()//2, arrow_y + 14))
        ...
        if self.task_editing:
            ...
            hint = "N: New  •  Space: Done  •  Enter: Expand  •  Tab: Filter  •  R: Sync"
        hint_surf = self.fonts['status'].render(hint, True, C['text_muted'])
        self.screen.blit(hint_surf, ((SCREEN_WIDTH - hint_surf.get_width())//2, SCREEN_HEIGHT - 18))
    
    def draw_tasks_superhuman(self):
        """Style 2: Superhuman Speed - Maximum density, vim keys, pure black"""
        import math
        ...
        # ═══════════════════════════════════════════════════════════════
        cmd_y = 48
        pygame.draw.rect(self.screen, (20, 20, 25), (10, cmd_y, SCREEN_WIDTH - 20, 32), border_radius=6)
        cmd_text = "/ Search..."
        cmd_surf = self.fonts['msg'].render(cmd_text, True, (80, 82, 95))
        self.screen.blit(cmd_surf, (24, cmd_y + 7))
        
        # Sync status on right
        sync_status = getattr(self, 'todoist_sync_status', 'live')
        if sync_status == 'live':
            sync_text = "● Synced"
            sync_color = (60, 180, 100)
        elif sync_status == 'syncing':
            sync_text = "◐ Sync..."
            sync_color = (180, 160, 60)
        else:
            sync_text = "✗ Error"
            sync_color = (200, 80, 80)
        sync_surf = self.fonts['status'].render(sync_text, True, sync_color)
        self.screen.blit(sync_surf, (SCREEN_WIDTH - sync_surf.get_width() - 20, cmd_y + 9))
        
        # ═══════════════════════════════════════════════════════════════
        # CATEGORY TABS - Number keys
        # ═══════════════════════════════════════════════════════════════
        tab_y = cmd_y + 42
        tabs = [
            {'id': 'all', 'key': '1', 'label': 'ALL', 'count': total},
            {'id': 'today', 'key': '2', 'label': 'TODAY', 'count': today_count},
            {'id': 'overdue', 'key': '3', 'label': 'LATE', 'count': overdue_count},
        ]
        ...
        for tab in tabs:
            ...
            # Key badge
            key_surf = self.fonts['status'].render(tab['key'], True, (100, 140, 220) if is_active else (60, 62, 75))
            self.screen.blit(key_surf, (tab_x, tab_y + 2))
            
            # Label
            label_color = (220, 225, 235) if is_active else (90, 92, 105)
            label_surf = self.fonts['msg'].render(tab['label'], True, label_color)
            self.screen.blit(label_surf, (tab_x + 18, tab_y))
            
            # Count
            count_surf = self.fonts['status'].render(str(tab['count']), True, (70, 72, 85))
            self.screen.blit(count_surf, (tab_x + 18 + label_surf.get_width() + 8, tab_y + 3))
            
            # Underline for active
            if is_active:
                line_w = label_surf.get_width() + 20
                pygame.draw.rect(self.screen, (100, 140, 220), (tab_x, tab_y + 24, line_w, 2))
            
            tab_x += 120
        
        # ═══════════════════════════════════════════════════════════════
        # TASK LIST - Minimal rows, high density
        # ═══════════════════════════════════════════════════════════════
        list_y = tab_y + 36
        row_h = 36
        max_visible = 8
        ...
        if not display_list:
            # Empty state
            empty_y = list_y + 80
            msg_surf = self.fonts['title'].render("Inbox Zero", True, (80, 180, 120))
            self.screen.blit(msg_surf, ((SCREEN_WIDTH - msg_surf.get_width())//2, empty_y))
        else:
            # Scroll handling
            if self.task_selected >= len(display_list):
                self.task_selected = max(0, len(display_list) - 1)
            ...
            for i, item in enumerate(visible):
                ...
                # Selection arrow
                if is_selected:
                    arrow_surf = self.fonts['msg'].render("→", True, (100, 140, 220))
                    self.screen.blit(arrow_surf, (15 + indent * 20, row_y + 8))
                
                # Priority color dot
                p_colors = {4: (235, 85, 85), 3: (235, 175, 55), 2: (85, 145, 235), 1: (55, 58, 72)}
                p_color = p_colors.get(priority, p_colors[1])
                if priority > 1:
                    pygame.draw.circle(self.screen, p_color, (40 + indent * 20, row_y + 18), 4)
                
                # Task text
                text_x = 55 + indent * 20
                text_color = (235, 238, 245) if is_selected else (180, 183, 195)
                max_len = 40 - indent * 5
                display_text = content[:max_len] + ('…' if len(content) > max_len else '')
                text_surf = self.fonts['msg'].render(display_text, True, text_color)
                self.screen.blit(text_surf, (text_x, row_y + 9))
                
                # Right side: Recurring, Assignee, Due
                right_x = SCREEN_WIDTH - 20
                
                # Due date
                if due:
                    due_text = due[:8]
                    due_color = (200, 70, 70) if 'overdue' in due.lower() else (120, 125, 140)
                    due_surf = self.fonts['status'].render(due_text, True, due_color)
                    right_x -= due_surf.get_width()
                    self.screen.blit(due_surf, (right_x, row_y + 11))
                    right_x -= 15
                
                # Recurring icon
                if is_recurring:
                    rec_surf = self.fonts['status'].render("↻", True, (100, 140, 200))
                    right_x -= rec_surf.get_width()
                    self.screen.blit(rec_surf, (right_x, row_y + 10))
                    right_x -= 10
                
                # Separator line
                if not is_selected:
                    pygame.draw.line(self.screen, (30, 32, 40), (50, row_y + row_h - 1), (SCREEN_WIDTH - 50, row_y + row_h - 1), 1)
                
                row_y += row_h
            
            # Scroll indicators
            if self.task_scroll > 0:
                up_surf = self.fonts['status'].render(f"↑ {self.task_scroll} more", True, (80, 82, 95))
                self.screen.blit(up_surf, (SCREEN_WIDTH//2 - up_surf.get_width()//2, list_y - 16))
            
            remaining = len(display_list) - self.task_scroll - max_visible
            if remaining > 0:
                down_surf = self.fonts['status'].render(f"↓ {remaining} more", True, (80, 82, 95))
                self.screen.blit(down_surf, (SCREEN_WIDTH//2 - down_surf.get_width()//2, row_y + 4))
        ...
        if self.task_editing:
            ...
            hint = "j/k: Move  •  x: Done  •  e: Edit  •  /: Search  •  ?: Help"
        hint_surf = self.fonts['status'].render(hint, True, (70, 72, 85))
        self.screen.blit(hint_surf, ((SCREEN_WIDTH - hint_surf.get_width())//2, SCREEN_HEIGHT - 18))
    
    def draw_tasks_things(self):
    ...
    def handle_key(self, event):
        ...
        # Ctrl+1/2/3 = Switch Task UI styles (only in Tasks mode)
        if self.mode == MODE_TASKS and event.mod & pygame.KMOD_CTRL:
            if event.key == pygame.K_1:
                self.settings.task_ui_style = 1
                self.settings.save()
                return
            elif event.key == pygame.K_2:
                self.settings.task_ui_style = 2
                self.settings.save()
                return
            elif event.key == pygame.K_3:
                self.settings.task_ui_style = 3
                self.settings.save()
                return
