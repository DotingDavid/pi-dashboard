#!/usr/bin/env python3
"""
Pi 400 Dashboard v6 - Real Terminal Edition
For 3.5" Waveshare TFT (480x320)

Uses pyte for proper VT100 terminal emulation.
The embedded terminal now works correctly with:
- Cursor positioning
- Line editing
- Colors (basic)
- Full-screen apps (openclaw tui, htop, vim, etc.)
"""
...
class PyteTerminal:
    """Terminal emulator using pyte for proper VT100 support"""
    ...
    def read(self):
        """Read from PTY and feed to pyte"""
    ...
    def get_display(self):
        """Get the screen content as list of lines with attributes"""
        self.read()
...
class DashboardApp:
    def __init__(self):
        ...
        pygame.display.set_caption('OpenClaw Dashboard v6')
        ...
        # Calculate terminal character size
    ...
    def draw_terminal_mode(self):
        """Draw terminal with pyte-based rendering"""
        ...
        self.text("(Ctrl+Q: quit to dashboard)", 'xs', C['text_dim'], 100, 48)
        ...
        if int(time.time() * 2) % 2:
            ...
            # Redraw character under cursor in reverse
    ...
    def handle_key(self, event):
        # Mode switching (only when not in terminal mode, or with Ctrl)
        if self.mode != MODE_TERMINAL:
            ...
            # In terminal mode, Ctrl+1/2/3 switch modes
        ...
        # Global quit
        if event.key == pygame.K_q and event.mod & pygame.KMOD_CTRL and self.mode != MODE_TERMINAL:
            return False
        ...
        # Scrolling (dashboard/tasks mode)
        if self.mode in [MODE_DASHBOARD, MODE_TASKS]:
        ...
        # Terminal input
        if self.mode == MODE_TERMINAL and self.terminal:
            # Special keys
            if event.key == pygame.K_RETURN:
                ...
                self.terminal.write('\x1b[5~')
            elif event.key == pygame.K_PAGEDOWN:
                self.terminal.write('\x1b[6~')
    ...
    def run(self):
        ...
        while running:
            ...
            # Auto-refresh (not in terminal mode to avoid interruption)
            if self.mode != MODE_TERMINAL and time.time() - self.last_refresh > REFRESH_INTERVAL:
                self.refresh_all()
            
            self.draw()
            
            # Higher FPS in terminal mode for responsiveness
            self.clock.tick(30 if self.mode == MODE_TERMINAL else 4)
