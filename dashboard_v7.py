...
# Terminal dimensions (adjusted for larger font)
TERM_COLS = 54
TERM_ROWS = 13
...
ANSI_COLORS = {
    'black': (40, 42, 54),
    'red': (255, 85, 85),
    'green': (120, 220, 140),  # Brighter green for better visibility
    'brown': (241, 250, 140),
    'blue': (120, 190, 255),  # Brighter blue for better visibility
    'magenta': (255, 121, 198),
    'cyan': (150, 240, 255),  # Brighter cyan
    'white': (248, 248, 242),
    'default': (220, 225, 235),  # Brighter default text
}
...
class ChatInterface:
    ...
    def _poll_for_response(self):
        ...
        response_file = Path.home() / '.openclaw' / 'workspace' / 'dashboard' / 'pi-chat-response.txt'
        
        while time.time() - start_time < max_wait:
            time.sleep(poll_interval)
            
            # First check for file-based response (workaround for broken CLI)
            try:
                if response_file.exists():
                    with open(response_file, 'r') as f:
                        content = f.read().strip()
                    if content:
                        # Got response via file!
                        response_file.unlink()  # Delete so we don't re-read it
                        if len(content) > 200:
                            content = content[:197] + "..."
                        self.add_message(content, is_user=False)
                        return
            except Exception as e:
                pass  # Silently continue if file read fails
            
            try:
                # Fallback: Read recent messages from channel (CLI is broken, usually times out)
                result = subprocess.run(
...
class DashboardApp:
    def __init__(self):
        ...
        self.fonts = {
            ...
            'term': pygame.font.SysFont('liberationmono', 12),  # Bigger for readability
        ...
        # State
        self.mode = MODE_TERMINAL  # Start in openclaw tui mode
        self.openclaw = {}
        self.system = {}
        self.network = {}
        self.weather = {}
    ...
    def draw_footer(self):
        ...
        if self.mode == MODE_TERMINAL:
            # Show helpful hints for openclaw tui
            self.text("Type to chat", 'xs', C['text_dim'], 10, y)
            self.text("Ctrl+Q: menu", 'xs', C['text_dim'], SCREEN_WIDTH - 10, y, right=True)
        else:
    ...
    def draw_terminal_mode(self):
        ...
        # Chat-styled header
        self.text("OpenClaw Chat", 'lg', C['text_bright'], 10, 44)
        now = datetime.now()
        self.text(now.strftime("%I:%M %p").lstrip('0'), 'sm', C['text_dim'], SCREEN_WIDTH - 10, 48, right=True)
        
        term_x = 4
        term_y = 68
        term_w = SCREEN_WIDTH - 8
        term_h = TERM_ROWS * self.char_height + 8
        
        # Darker background for better contrast
        pygame.draw.rect(self.screen, C['bg'], (term_x, term_y, term_w, term_h), border_radius=4)
